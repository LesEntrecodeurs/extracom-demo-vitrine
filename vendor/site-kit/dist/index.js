...
function createSiteKit(config) {
  const cfg = config ?? serverConfigFromEnv();
...
function formatPrice(value, opts) {
  if (value == null) return (opts == null ? void 0 : opts.whenHidden) ?? "\u2014";
  return new Intl.NumberFormat((opts == null ? void 0 : opts.locale) ?? "fr-FR", {
    style: "currency",
    currency: (opts == null ? void 0 : opts.currency) ?? "EUR"
  }).format(value);
}
function parseDate(value) {
  const sage = value.match(/\/Date\((-?\d+)(?:[+-]\d{4})?\)\//);
  const d = sage ? new Date(Number(sage[1])) : new Date(value);
...
function formatDate(value, opts) {
  ...
  if (!d) return value;
  return d.toLocaleDateString((opts == null ? void 0 : opts.locale) ?? "fr-FR", {
    day: "2-digit",
    month: "short",
    year: "numeric"
  });
...
exports.SiteApiError = SiteApiError;
exports.SiteMaintenanceError = SiteMaintenanceError;
exports.createSiteKit = createSiteKit;
