...
function secret() {
  ...
  if (!s) {
    throw new Error("[site-kit] EXTRACOM_SESSION_SECRET requis (env serveur).");
  }
  return s;
}
...
async function getArticleFieldValuesAction(field, baseQuery) {
  return kit().catalog.getFieldValues(field, await resolveCtx(), baseQuery);
}
...
async function getAnonymousArticleFieldValuesAction(field, baseQuery) {
  return kit().catalog.getFieldValues(field, {}, baseQuery);
}
...
exports.getAnonymousArticleFieldValuesAction = getAnonymousArticleFieldValuesAction;
...
exports.getArticleFieldValuesAction = getArticleFieldValuesAction;
